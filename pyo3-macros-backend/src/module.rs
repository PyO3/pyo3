...
pub fn pymodule_module_impl(mut module: syn::ItemMod) -> Result<TokenStream> {
    ...
    Ok(quote!(
        #vis mod #ident {
            ...
            fn __pyo3_pymodule(_py: #krate::Python, module: &#krate::types::PyModule) -> #krate::PyResult<()> {
                use impl_::PyAddToModule;
                #(
    ...
    Ok(quote! {
        ...
        const _: () = {
            ...
            impl #ident::MakeDef {
                const fn make_def() -> impl_::ModuleDef {
                    unsafe {
                        const INITIALIZER: impl_::ModuleInitializer = impl_::ModuleInitializer(#ident);
                        impl_::ModuleDef::new(
                            #ident::__PYO3_NAME,
                            #doc,
                            INITIALIZER
