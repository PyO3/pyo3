...
pub unsafe fn Py_REFCNT(ob: *mut PyObject) -> Py_ssize_t {
    #[cfg(Py_GIL_DISABLED)]
    {
        let local = _Py_atomic_load_uint32_relaxed((*ob).ob_ref_local.as_ptr());
        ...
        let shared = _Py_atomic_load_ssize_relaxed((*ob).ob_ref_shared.as_ptr());
...
unsafe fn _Py_IsImmortal(op: *mut PyObject) -> c_int {
    ...
    #[cfg(Py_GIL_DISABLED)]
    {
        (_Py_atomic_load_uint32_relaxed((*op).ob_ref_local.as_ptr()) == _Py_IMMORTAL_REFCNT_LOCAL)
            as c_int
    }
}
