...
pub static mut SEQUENTIAL_SLOTS: [PySlot; SEQUENTIAL_SLOTS_LEN] = [
    ...
    PySlot_FUNC(
        ...
        Some(sequential_traverse as *mut c_void),
    ),
    PySlot_FUNC(Py_mod_state_clear, Some(sequential_clear as *mut c_void)),
    PySlot_FUNC(Py_mod_state_free, Some(sequential_free as *mut c_void)),
    PySlot_FUNC(Py_mod_exec, Some(sequential_exec as *mut c_void)),
    PySlot_DATA(
