...
impl PyErr {
    ...
    /// Fetches the current error from the Python interpreter's global state and clears the global
    ...
    /// Similar to [PyErr::fetch], to be used when it is known that an error is set in the Python
    /// interpreter state, such as when a C FFI return value indicates an error.
    ///
    /// If no error is set:
    ...
    /// value indicates an error occurred but the global error state is empty.
    #[cfg_attr(all(debug_assertions, track_caller), track_caller)]
    #[inline]
    pub fn fetch_last_error(py: Python) -> PyErr {
        const FAILED_TO_FETCH: &str = "attempted to fetch last error but none was set";
        match PyErr::fetch(py) {
            Some(err) => err,
...
mod tests {
    ...
    #[test]
    fn no_error() {
        assert!(Python::with_gil(PyErr::fetch).is_none());
    }
    ...
    fn set_valueerror() {
        Python::with_gil(|py| {
            ...
            let err = PyErr::fetch_last_error(py);
            assert!(err.is_instance::<exceptions::PyValueError>(py));
    ...
    fn set_typeerror() {
        ...
        err.restore(py);
        assert!(PyErr::occurred(py));
        drop(PyErr::fetch_last_error(py));
    }
    ...
    fn fetching_panic_exception_resumes_unwind() {
        ...
        Python::with_gil(|py| {
            ...
            let _ = PyErr::fetch_last_error(py);
        });
    }
