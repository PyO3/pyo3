...
macro_rules! impl_native_exception (
    ($rust_name:ident, $exc_name:ident, $python_name:expr, $doc:expr, $layout:path $(, #checkfunction=$checkfunction:path)?) => (
        #[doc = $doc]
        #[repr(transparent)]
        #[allow(clippy::upper_case_acronyms)]
        pub struct $rust_name($crate::PyAny);

        $crate::impl_exception_boilerplate!($rust_name);
        $crate::pyobject_native_type!($rust_name, $layout, |_py| unsafe { $crate::ffi::$exc_name as *mut $crate::ffi::PyTypeObject }, $python_name $(, #checkfunction=$checkfunction)?);
        $crate::pyobject_subclassable_native_type!($rust_name, $layout);
    );
    ($rust_name:ident, $exc_name:ident, $python_name:expr, $doc:expr) => (
        impl_native_exception!($rust_name, $exc_name, $python_name, $doc, $crate::ffi::PyBaseExceptionObject);
    )
);

#[cfg(windows)]
macro_rules! impl_windows_native_exception (
    ($rust_name:ident, $exc_name:ident, $python_name:expr, $doc:expr, $layout:path) => (
        #[cfg(windows)]
        #[doc = $doc]
        #[repr(transparent)]
        #[allow(clippy::upper_case_acronyms)]
        ...
        $crate::impl_exception_boilerplate!($rust_name);
        $crate::pyobject_native_type!($rust_name, $layout, |_py| unsafe { $crate::ffi::$exc_name as *mut $crate::ffi::PyTypeObject }, $python_name);
    );
    ($rust_name:ident, $exc_name:ident, $python_name:expr, $doc:expr) => (
        impl_windows_native_exception!($rust_name, $exc_name, $python_name, $doc, $crate::ffi::PyBaseExceptionObject);
    )
);
