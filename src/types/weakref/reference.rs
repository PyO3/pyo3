...
impl PyWeakrefReference {
    ...
    ///     let weakref = PyWeakrefReference::new(foo.as_any())?;
    ///     assert!(
    ...
    ///     );
    ///
    ///     let weakref2 = PyWeakrefReference::new(foo.as_any())?;
    ...
    ///
    ///     // This is fine.
    ///     let weakref = PyWeakrefReference::new_with(foo.as_any(), py.None())?;
    ...
    ///     assert_eq!(py.eval(c_str!("counter"), None, None)?.extract::<u32>()?, 0);
    ///
    ///     let weakref2 = PyWeakrefReference::new_with(foo.as_any(), wrap_pyfunction!(callback, py)?)?;
    ...
    mod pyo3_pyclass {
        ...
        fn test_weakref_upgrade_as() -> PyResult<()> {
            Python::with_gil(|py| {
                ...
                let reference = PyWeakrefReference::new(object.bind(py).as_any())?;
