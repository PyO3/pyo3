...
macro_rules! pyobject_type_info_type_hint(
    ($module:expr, $name:expr $(, #type_hint=$type_hint:expr)?) => {};
);
...
macro_rules! pyobject_type_info_type_hint(
    ...
    ($module:expr, $name:expr, #type_hint=$type_hint:expr) => {
        const TYPE_HINT: $crate::inspect::PyStaticExpr = $crate::type_hint_identifier!($module, $name);
        const STANDALONE_TYPE_HINT: $crate::inspect::PyStaticExpr = $type_hint;
    };
);
...
macro_rules! pyobject_native_type_info(
    ($name:ty, $typeobject:expr, $type_hint_module:expr, $type_hint_name:expr, $module:expr $(, #checkfunction=$checkfunction:path)? $(, #type_hint=$type_hint:expr)? $(;$generics:ident)*) => {
        // SAFETY: macro caller has upheld the safety contracts
        unsafe impl<$($generics,)*> $crate::type_object::PyTypeInfo for $name {
            const NAME: &'static str = stringify!($name);
            const MODULE: ::core::option::Option<&'static str> = $module;
            $crate::pyobject_type_info_type_hint!($type_hint_module, $type_hint_name $(, #type_hint=$type_hint)?);
...
macro_rules! pyobject_native_type_core {
    ($name:ty, $typeobject:expr, $type_hint_module:expr, $type_hint_name:expr, #module=$module:expr $(, #checkfunction=$checkfunction:path)? $(, #type_hint=$type_hint:expr)? $(;$generics:ident)*) => {
        $crate::pyobject_native_type_named!($name $(;$generics)*);
        $crate::pyobject_native_type_info!($name, $typeobject, $type_hint_module, $type_hint_name, $module $(, #checkfunction=$checkfunction)? $(, #type_hint=$type_hint)? $(;$generics)*);
    };
    ...
    ($name:ty, $typeobject:expr, $type_hint_module:expr, $type_hint_name:expr $(, #checkfunction=$checkfunction:path)? $(, #type_hint=$type_hint:expr)? $(;$generics:ident)*) => {
        $crate::pyobject_native_type_core!($name, $typeobject, $type_hint_module, $type_hint_name, #module=::core::option::Option::Some("builtins") $(, #checkfunction=$checkfunction)? $(, #type_hint=$type_hint)? $(;$generics)*);
    };
}
...
macro_rules! pyobject_native_type {
    ($name:ty, $layout:path, $typeobject:expr, $type_hint_module:expr, $type_hint_name:expr $(, #module=$module:expr)? $(, #checkfunction=$checkfunction:path)? $(, #type_hint=$type_hint:expr)? $(;$generics:ident)*) => {
        $crate::pyobject_native_type_core!($name, $typeobject, $type_hint_module, $type_hint_name $(, #module=$module)? $(, #checkfunction=$checkfunction)? $(, #type_hint=$type_hint)? $(;$generics)*);
