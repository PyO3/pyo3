...
/// [2]: AsyncioWaker::wake
pub struct AsyncioWaker(PyOnceCell<Option<LoopAndFuture>>);
...
impl LoopAndFuture {
    fn new(py: Python<'_>) -> PyResult<Self> {
        static GET_RUNNING_LOOP: PyOnceCell<PyObject> = PyOnceCell::new();
    ...
    fn set_result(&self, py: Python<'_>) -> PyResult<()> {
        static RELEASE_WAITER: PyOnceCell<Py<PyCFunction>> = PyOnceCell::new();
