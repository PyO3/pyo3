...
const NOT_IN_COROUTINE_CONTEXT: &str = "Python awaitable must be awaited in coroutine context";
        ...
        match obj.call_method0(__await__) {
            ...
            Err(err) if err.is_instance_of::<PyAttributeError>(obj.py()) => {
                if obj.hasattr(__await__)? {
                    ...
                    Ok(Self(PyIterator::from_object(obj)?.unbind().into_any()))
                } else {
...
impl Future for AwaitImpl {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        match try_delegate(cx.waker(), Python::with_gil(|py| self.0.clone_ref(py))) {
            Some(poll) => poll,
...
///
/// ```rust
/// use pyo3::{coroutine::await_in_coroutine, prelude::*, py_run, wrap_pyfunction};
///
/// # fn main() {
/// #[pyfunction]
...
/// }
/// Python::with_gil(|py| {
///     let wrap_awaitable = wrap_pyfunction!(wrap_awaitable, py).unwrap();
///     let test = r#"
///         import asyncio
