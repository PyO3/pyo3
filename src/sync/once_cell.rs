use crate::{gil::SuspendGIL, types::any::PyAnyMethods, Bound, Py, PyResult, PyTypeCheck, Python};
...
/// A wrapper for [`once_cell::sync::OnceCell<T>`] for initializing Python objects
///
...
impl<T> PyOnceCell<T> {
    ...
    where
        F: FnOnce() -> T,
    {
        self.inner.get_or_init_py_attached(py, f)
    }
    ...
    where
        F: FnOnce() -> Result<T, E>,
    {
        self.inner.get_or_try_init_py_attached(py, f)
    }
