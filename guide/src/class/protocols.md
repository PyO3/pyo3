...
impl MyIterator {
    ...
    fn __next__(slf: PyRefMut<'_, '_, Self>) -> Option<PyObject> {
...
(i.e. the _iterable_) and the iterator it provides. In this case, the iterable
...
impl Iter {
    ...
    fn __next__(mut slf: PyRefMut<'_, '_, Self>) -> Option<usize> {
        slf.inner.next()
    }
}
...
#[pymethods]
impl Container {
    fn __iter__(slf: PyRef<'_, '_, Self>) -> PyResult<Py<Iter>> {
        let iter = Iter {
...
- `__getitem__(<self>, object) -> object`
  ...
  _Note:_ Negative integer indexes are not handled specially by PyO3.
