...
pub fn get() -> &'static InterpreterConfig {
    ...
    CONFIG.get_or_init(|| {
        ...
        if let Some(interpreter_config) = InterpreterConfig::from_cargo_dep_env() {
            interpreter_config
        } else if let Some(interpreter_config) =
            config_from_pyo3_config_file_env(&impl_::target_triple_from_env())
        {
...
///
/// May apply target-specific fixups, such as a default lib name or generate import libs on Windows.
#[cfg(feature = "resolve-config")]
fn config_from_pyo3_config_file_env(triple: &Triple) -> Option<InterpreterConfig> {
    #[doc(hidden)]
    const CONFIG_FILE: &str = include_str!(concat!(env!("OUT_DIR"), "/pyo3-build-config-file.txt"));

    // CONFIG_FILE is generated in build.rs, so its content can vary
    // TODO: `unknown_lints` allow not needed on MSRV 1.79+
    #[allow(unknown_lints, clippy::const_is_empty)]
    if !CONFIG_FILE.is_empty() {
        let mut config = InterpreterConfig::from_reader(Cursor::new(CONFIG_FILE))
            .expect("contents of CONFIG_FILE should always be valid (generated by pyo3-build-config's build.rs)");
        config.apply_default_lib_name_to_config_file(&triple);
        Some(config)
    } else {
        None
    }
...
pub mod pyo3_build_script_impl {
    ...
    /// 1. If `PYO3_CONFIG_FILE` is set, the windows import libs might be generated.
    ...
    pub fn resolve_build_config(target: &Triple) -> Result<BuildConfig> {
        ...
        #[allow(unknown_lints, clippy::const_is_empty)]
        if let Some(mut interpreter_config) = config_from_pyo3_config_file_env(target) {
