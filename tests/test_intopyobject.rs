...
fn test_generic_with_bound() {
    Python::with_gil(|py| {
        ...
        assert_eq!(PyDict::len(&map), 2);
        assert_eq!(
            PyDict::get_item(&map, "1")
                .unwrap()
                .unwrap()
                .extract::<i32>()
                .unwrap(),
        ...
        assert!(PyDict::get_item(&map, "3").unwrap().is_none());
    });
}
