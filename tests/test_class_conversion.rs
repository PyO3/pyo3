...
fn test_cloneable_pyclass() {
    ...
    Python::with_gil(|py| {
        ...
        assert_eq!(c, c2);
        {
            let rc: PyRef<'_, '_, Cloneable> = py_c.extract(py).unwrap();
            assert_eq!(&c, &*rc);
            // Drops PyRef before taking PyRefMut
        }
        let mrc: PyRefMut<'_, '_, Cloneable> = py_c.extract(py).unwrap();
        assert_eq!(&c, &*mrc);
    });
}
...
fn test_pyref_as_base() {
    Python::with_gil(|py| {
        ...
        // First try PyRefMut
        let sub: PyRefMut<'_, '_, SubClass> = cell.borrow_mut();
        let mut base: PyRefMut<'_, '_, BaseClass> = sub.into_super();
        ...
        // Repeat for PyRef
        let sub: PyRef<'_, '_, SubClass> = cell.borrow();
        let base: PyRef<'_, '_, BaseClass> = sub.into_super();
        assert_eq!(999, base.value);
    });
}
