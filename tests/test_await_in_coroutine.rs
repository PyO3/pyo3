...
fn awaitable() {
    Python::with_gil(|gil| {
        ...
        let run = |awaitable| {
            gil.run(
                &CString::new(common::asyncio_windows(test).replace("...", awaitable)).unwrap(),
                Some(&globals),
                None,
            )
        };
...
#[test]
#[should_panic(expected = "Python awaitable must be awaited in coroutine context")]
fn awaitable_without_coroutine() {
    #[pyfunction]
